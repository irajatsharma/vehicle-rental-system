...
if(isset($_POST['enter'])){
  include 'setup.php';
   ...
   $my_email=$_SESSION['user_email'];
    $qry1="SELECT * FROM car_issue WHERE email='$my_email'";
    $run1=mysqli_query($conn,$qry1);
    while($row1=mysqli_fetch_array($run1)){
    $date2=$row1['to_date_car'];
    ...
    if($newday > $date){
      ...
      $qry2= "INSERT INTO car_billing(customer_name,from_date_car,to_date_car,email,late_fees_car) VALUES ('$customer_name','$date1','$return_date','$my_email','$tax')";
      $run2=mysqli_query($conn,$qry2);
      session_start();
              $_SESSION['user_email']=$my_email;
      echo "<script type = \"text/javascript\">
                         alert(\"Couldnt Hire...\");
                         window.location = (\"billing_car.php\")
                           </script>";
    }
    else
    {  $tax="0";
