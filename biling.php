...
<body>
	<table align="center">
		...
		<?php
          ...
          $qry="SELECT * FROM bike_billing WHERE email='$my_email'";
          $run=mysqli_query($conn,$qry);
          while($row=mysqli_fetch_array($run)){
          	$date2=$row['to_date_bike'];
          	$date1=$row['from_date_bike'];
          	$late=$row['late_fees'];
          	$name=$row['customer_name_bike'];
          }
          $qry3="SELECT * FROM bike_issue WHERE email='$my_email'";
